...
//
// incidence(g,u) -> edge_view<VId,Sourced,E,EV>:
//
// enable: for([vid, uv]        : incidence(g,uid))
//         for([vid, uv, value] : incidence(g,uid,fn))
//
//         for([uid, vid, uv]        : sourced_incidence(g,uid))
//         for([uid, vid, uv, value] : sourced_incidence(g,uid,fn))
//
