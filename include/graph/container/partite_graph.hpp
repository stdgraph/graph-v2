...
// PARTITE NOTES
//  Allow n vertex types, where each type also identifies the allowed target types
...
//
// partite_graph(initializer_list<[uid,vid,eval]>) : load_edges(erng,eproj)
// partite_graph(erng, eproj) : load_edges(erng,eproj)
// partite_graph(erng, eproj, vrng, vproj): load(erng, eproj, vrng, vprog)
//
...
class partite_row_values {
  using row_type           = partite_row<EIndex>; // index into col_index_
...
public:
  using graph_type        = partite_graph<EV, VV, GV, VId, EIndex, Alloc>;
  ...
  constexpr partite_row_values(const Alloc& alloc) : v_(alloc) {}

  constexpr partite_row_values()                          = default;
  constexpr partite_row_values(const partite_row_values&) = default;
  constexpr partite_row_values(partite_row_values&&)      = default;
  constexpr ~partite_row_values()                         = default;

  constexpr partite_row_values& operator=(const partite_row_values&) = default;
  constexpr partite_row_values& operator=(partite_row_values&&)      = default;
...
public: // Operations
  ...
  constexpr void swap(partite_row_values& other) noexcept { swap(v_, other.v_); }
...
private:
  ...
  tag_invoke(::std::graph::tag_invoke::vertex_value_fn_t, const graph_type& g, const vertex_type& u) {
    static_assert(ranges::contiguous_range<row_index_vector>, "row_index_ must be a contiguous range to evaluate uidx");
    auto                      uidx     = g.index_of(u);
    const partite_row_values& row_vals = g;
    return row_vals.v_[uidx];
  }
...
class partite_row_values<EV, void, GV, VId, EIndex, Alloc> {
public:
  constexpr partite_row_values(const Alloc& alloc) {}
  constexpr partite_row_values() = default;
...
public: // Operations
  ...
  constexpr void swap(partite_row_values& other) noexcept {}
};
...
*/
template <class EV, class VV, class GV, integral VId, integral EIndex, class Alloc>
class partite_col_values {
  using col_type           = partite_col<VId>; // target_id
...
public:
  ...
  constexpr partite_col_values(const Alloc& alloc) : v_(alloc) {}

  constexpr partite_col_values()                          = default;
  constexpr partite_col_values(const partite_col_values&) = default;
  constexpr partite_col_values(partite_col_values&&)      = default;
  constexpr ~partite_col_values()                         = default;

  constexpr partite_col_values& operator=(const partite_col_values&) = default;
  constexpr partite_col_values& operator=(partite_col_values&&)      = default;
...
public: // Operations
  ...
  constexpr void swap(partite_col_values& other) noexcept { swap(v_, other.v_); }
...
private:
  ...
  tag_invoke(::std::graph::tag_invoke::edge_value_fn_t, const graph_type& g, const edge_type& uv) {
    auto                      uv_idx   = g.index_of(uv);
    const partite_col_values& col_vals = g;
    return col_vals.v_[uv_idx];
  }
...
class partite_col_values<void, VV, GV, VId, EIndex, Alloc> {
public:
  constexpr partite_col_values(const Alloc& alloc) {}
  constexpr partite_col_values() = default;
...
public: // Operations
  ...
  constexpr void clear() noexcept {}
  constexpr void swap(partite_col_values& other) noexcept {}
};
...
*/
template <class EV, class VV, class GV, integral VId, integral EIndex, class Alloc>
class partite_graph_base
      : public partite_row_values<EV, VV, GV, VId, EIndex, Alloc>
      , public partite_col_values<EV, VV, GV, VId, EIndex, Alloc> {
  using row_values_base = partite_row_values<EV, VV, GV, VId, EIndex, Alloc>;
  using col_values_base = partite_col_values<EV, VV, GV, VId, EIndex, Alloc>;
...
public: // Types
  using graph_type = partite_graph_base<EV, VV, GV, VId, EIndex, Alloc>;
...
public: // Construction/Destruction
  constexpr partite_graph_base()                          = default;
  constexpr partite_graph_base(const partite_graph_base&) = default;
  constexpr partite_graph_base(partite_graph_base&&)      = default;
  constexpr ~partite_graph_base()                         = default;

  constexpr partite_graph_base& operator=(const partite_graph_base&) = default;
  constexpr partite_graph_base& operator=(partite_graph_base&&)      = default;

  constexpr partite_graph_base(const Alloc& alloc)
  ...
  */
  ...
  requires copyable_edge<invoke_result<EProj, ranges::range_value_t<ERng>>, VId, EV>
  constexpr partite_graph_base(const ERng& erng, EProj eprojection = {}, const Alloc& alloc = Alloc())
        : row_values_base(alloc), col_values_base(alloc), row_index_(alloc), col_index_(alloc) {
  ...
  */
  ...
  constexpr partite_graph_base(const ERng&  erng,
                               const VRng&  vrng,
   ...
   * @param alloc   Allocator to use for internal containers
  */
  constexpr partite_graph_base(const initializer_list<copyable_edge_t<VId, EV>>& ilist, const Alloc& alloc = Alloc())
        : row_values_base(alloc), col_values_base(alloc), row_index_(alloc), col_index_(alloc) {
...
public:                            // Operations
  ...
  */
  ...
  constexpr void load_edges(ERng&& erng, EProj eprojection = {}, size_type vertex_count = 0, size_type edge_count = 0) {
    ...
    for (auto&& edge_data : erng) {
      auto&& edge = eprojection(edge_data); // partite_graph requires EV!=void
...
private: // tag_invoke properties
  friend constexpr vertices_type tag_invoke(::std::graph::tag_invoke::vertices_fn_t, partite_graph_base& g) {
  ...
  tag_invoke(::std::graph::tag_invoke::vertex_id_fn_t, const partite_graph_base& g, const_iterator ui) {
...
*/
template <class EV, class VV, class GV, integral VId, integral EIndex, class Alloc>
class partite_graph : public partite_graph_base<EV, VV, GV, VId, EIndex, Alloc> {
public: // Types
  using graph_type = partite_graph<EV, VV, GV, VId, EIndex, Alloc>;
  using base_type  = partite_graph_base<EV, VV, GV, VId, EIndex, Alloc>;
...
public: // Construction/Destruction
  constexpr partite_graph()                     = default;
  constexpr partite_graph(const partite_graph&) = default;
  constexpr partite_graph(partite_graph&&)      = default;
  constexpr ~partite_graph()                    = default;

  constexpr partite_graph& operator=(const partite_graph&) = default;
  constexpr partite_graph& operator=(partite_graph&&)      = default;

  // partite_graph(      alloc)
  // partite_graph(gv&,  alloc)
  // partite_graph(gv&&, alloc)

  constexpr partite_graph(const Alloc& alloc) : base_type(alloc) {}
  constexpr partite_graph(const graph_value_type& value, const Alloc& alloc = Alloc())
        : base_type(alloc), value_(value) {}
  constexpr partite_graph(graph_value_type&& value, const Alloc& alloc = Alloc())
        : base_type(alloc), value_(move(value)) {}

  // partite_graph(      erng, eprojection, alloc)
  // partite_graph(gv&,  erng, eprojection, alloc)
  // partite_graph(gv&&, erng, eprojection, alloc)

  template <ranges::forward_range ERng, class EProj = identity>
  requires copyable_edge<invoke_result<EProj, ranges::range_value_t<ERng>>, VId, EV>
  constexpr partite_graph(const ERng& erng, EProj eprojection, const Alloc& alloc = Alloc())
        : base_type(erng, eprojection, alloc) {}

  template <ranges::forward_range ERng, class EProj = identity>
  requires copyable_edge<invoke_result<EProj, ranges::range_value_t<ERng>>, VId, EV>
  constexpr partite_graph(const graph_value_type& value,
                          const ERng&             erng,
                          EProj                   eprojection,
                          const Alloc&            alloc = Alloc())
  ...
  template <ranges::forward_range ERng, class EProj = identity>
  requires copyable_edge<invoke_result<EProj, ranges::range_value_t<ERng>>, VId, EV>
  constexpr partite_graph(graph_value_type&& value, const ERng& erng, EProj eprojection, const Alloc& alloc = Alloc())
        : base_type(erng, eprojection, alloc), value_(move(value)) {}

  // partite_graph(      erng, vrng, eprojection, vprojection, alloc)
  // partite_graph(gv&,  erng, vrng, eprojection, vprojection, alloc)
  // partite_graph(gv&&, erng, vrng, eprojection, vprojection, alloc)

  template <ranges::forward_range ERng, ranges::forward_range VRng, class EProj = identity, class VProj = identity>
  requires copyable_edge<invoke_result<EProj, ranges::range_value_t<ERng>>, VId, EV> &&
           copyable_vertex<invoke_result<VProj, ranges::range_value_t<VRng>>, VId, VV>
  constexpr partite_graph(const ERng&  erng,
                          const VRng&  vrng,
                          EProj        eprojection = {},
                          VProj        vprojection = {},
                          const Alloc& alloc       = Alloc())
  ...
  requires copyable_edge<invoke_result<EProj, ranges::range_value_t<ERng>>, VId, EV> &&
                 copyable_vertex<invoke_result<VProj, ranges::range_value_t<VRng>>, VId, VV>
  constexpr partite_graph(graph_value_type&& value,
                          const ERng&        erng,
                          const VRng&        vrng,
                          EProj              eprojection = {},
                          VProj              vprojection = {},
  ...
  constexpr partite_graph(const initializer_list<copyable_edge_t<VId, EV>>& ilist, const Alloc& alloc = Alloc())
...
class partite_graph<EV, VV, void, VId, EIndex, Alloc> : public partite_graph_base<EV, VV, void, VId, EIndex, Alloc> {
public: // Types
  using graph_type = partite_graph<EV, VV, void, VId, EIndex, Alloc>;
  using base_type  = partite_graph_base<EV, VV, void, VId, EIndex, Alloc>;
