...
public: // Construction/Destruction
  ...
  */
   ...
   * @param alloc   Allocator to use for internal containers
  */
  constexpr csr_graph_base(const std::initializer_list<copyable_edge_t<VId, EV>>& ilist,
                                  const Alloc&                                           alloc = Alloc())
        : row_values_base(alloc), col_values_base(alloc), row_index_(alloc), col_index_(alloc), partition_(alloc) {
...
private: // CPO properties
  friend constexpr vertices_type vertices(csr_graph_base& g) {
  ...
  friend constexpr const_vertices_type vertices(const csr_graph_base& g) {
    if (g.row_index_.empty())
  ...
  friend constexpr auto num_edges(const csr_graph_base& g) {
    return static_cast<size_type>(g.col_index_.size());
  }
  friend constexpr bool has_edge(const csr_graph_base& g) { return g.col_index_.size() > 0; }

  friend vertex_id_type vertex_id(const csr_graph_base& g, const_iterator ui) {
  ...
  friend constexpr auto num_partitions(const csr_graph_base& g) {
    return static_cast<partition_id_type>(g.partition_.size() - 1);
  }

  friend constexpr auto partition_id(const csr_graph_base& g, vertex_id_type uid) {
  ...
  friend constexpr auto num_vertices(const csr_graph_base& g, partition_id_type pid) {
    assert(static_cast<size_t>(pid) < g.partition_.size() - 1);
    g.partition_[pid + 1] - g.partition_[pid];
  }

  friend constexpr auto vertices(const csr_graph_base& g, partition_id_type pid) {
    assert(static_cast<size_t>(pid) < g.partition_.size() - 1);
...
public: // Construction/Destruction
  ...
  constexpr csr_graph(const graph_value_type& value, const Alloc& alloc = Alloc())
        : base_type(alloc), value_(value) {}
  constexpr csr_graph(graph_value_type&& value, const Alloc& alloc = Alloc())
  ...
  constexpr csr_graph(const std::initializer_list<copyable_edge_t<VId, EV>>& ilist, const Alloc& alloc = Alloc())
...
class csr_graph<EV, VV, void, VId, EIndex, Alloc>
      : public csr_graph_base<EV, VV, void, VId, EIndex, Alloc> {
public: // Types
