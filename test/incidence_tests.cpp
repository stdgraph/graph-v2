...
TEST_CASE("incidence test", "[csr][incidence]") {
  ...
  SECTION("non-const incidence") {
    ...
    // Subrange of edges not common
    //cnt = 0;
    //for (auto&& [vkey, uv] : std::graph::views::incidence(g, begin(edges(g,u)), end(edges(g,u)))) {
    //  ++cnt;
    //}
    //REQUIRE(cnt == size(edges(g,u)));
  }
  ...
  SECTION("const incidence") {
    ...
    // Subrange of edges not common
    //cnt = 0;
    //for (auto&& [vkey, uv] : std::graph::views::incidence(g2, begin(vertices(g2)), end(vertices(g2)))) {
    //  ++cnt;
    //}
    //REQUIRE(cnt == size(edges(g,u)));
  }
