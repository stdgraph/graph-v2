...
\begin{enumerate}
\item All functions are Customization Point Objects (CPO) that can be overridden to integrate it with this library. 6-9 functions must be
overridden for a graph data structure to fully integrate with this library, and more functions are available if needed. This proposal includes 
...
The discrepancy between these definitions is to enable a wider variety of algorithms andm graph data structures outside the standard that 
...
\hspace*{30mm}\texttt{graph::access}\\
\hspace*{30mm}\texttt{graph::views}\\
\hspace*{30mm}\texttt{graph::views::access}\\
\\
Proposed headers include\\
\hspace*{20mm}\texttt{<graph>}\\
\hspace*{30mm}\texttt{<graph\_view>}\\
\hspace*{30mm}\texttt{<graph\_algorithm>}\\
