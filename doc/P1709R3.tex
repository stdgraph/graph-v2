...
\hline
    ...
    \tcode{VId} & \tcode{vertex_id_t<G>} & \tcode{uid, vid} & Vertex id. \tcode{uid} is the source (or only) vertex id. \tcode{vid} is the target vertex id. \\
    \tcode{VV} &  \tcode{vertex_value_t<G>} & & Vertex Value, value or reference. This can be either the user-defined value on a vertex, or a value returned by a function object (e.g. \tcode{VVF}) that is related to the vertex. \\
    \tcode{VR} &  \tcode{vertex_range_t<G>} & & Vertex Range \\
    \tcode{VI} & \tcode{vertex_iterator_t<G>} & \tcode{ui, vi} & Vertex Iterator. \tcode{ui} is the source (or only) vertex. \tcode{vi} is the target vertex. \\
...
\hline
    ...
    \tcode{EV} &  \tcode{edge_value_t<G>} & & Edge Value, value or reference. This can be either the user-defined value on an edge, or a value returned by a function object (e.g. \tcode{EVF}) that is related to the edge. \\
...
\hline
    ...
    \tcode{vertex_view<VId, V>} & \tcode{id} & \tcode{vertex} & \\
    \tcode{vertex_view<VId, VV>} & \tcode{id} & & \tcode{value} \\
    \tcode{vertex_view<VId>} & \tcode{id} & & \\
\hline
\end{tabular}}
...
\hline
    ...
    \tcode{edge_view<VId, true, E>} & \tcode{source_id} & \tcode{target_id} & \tcode{edge} & \\
    \tcode{edge_view<VId, true, EV>} & \tcode{source_id} & \tcode{target_id} & & \tcode{value} \\
    \tcode{edge_view<VId, true>} & \tcode{source_id} & \tcode{target_id} & & \\
    \tcode{edge_view<VId, false, E, EV>} & & \tcode{target_id} & \tcode{edge} & \tcode{value} \\
    \tcode{edge_view<VId, false, E>} & & \tcode{target_id} & \tcode{edge} & \\
    \tcode{edge_view<VId, false, EV>} & & \tcode{target_id} & & \tcode{value} \\
    \tcode{edge_view<VId, false>} & & \tcode{target_id} & & \\
\hline
\end{tabular}}
...
\hline
    ...
    \multicolumn{4}{c}{\textbf{Members}} \\
\hline
    \tcode{neighbor_view<VId, true, E, EV>} & \tcode{source_id} & \tcode{target_id} & \tcode{neighbor} & \tcode{value} \\
    \tcode{neighbor_view<VId, true, E>} & \tcode{source_id} & \tcode{target_id} & \tcode{neighbor} & \\
    \tcode{neighbor_view<VId, true, EV>} & \tcode{source_id} & \tcode{target_id} & & \tcode{value} \\
    \tcode{neighbor_view<VId, true>} & \tcode{source_id} & \tcode{target_id} & & \\
    \tcode{neighbor_view<VId, false, E, EV>} & & \tcode{target_id} & \tcode{neighbor} & \tcode{value} \\
    \tcode{neighbor_view<VId, false, E>} & & \tcode{target_id} & \tcode{neighbor} & \\
    \tcode{neighbor_view<VId, false, EV>} & & \tcode{target_id} & & \tcode{value} \\
    \tcode{neighbor_view<VId, false>} & & \tcode{target_id} & & \\
\hline
\end{tabular}}
...
\phil{Missing overloads for vertex range}
