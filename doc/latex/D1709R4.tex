...
\lstset
{
  ...
  basicstyle=\ttfamily\small, % \bf\ttfamily,
  breaklines=true,
...
Dijkstra's algorithm \cite{REF_} is a single-source, shortest paths algorithm. It finds the shortest paths and their weighted distances to all 
vertices connected to a single seed vertex.
...
{\begin{tabular}{l l}
\hline
    \textit{Complexity} & O(E+2V), where E is the number of edges connected to \tcode{seed}, directly or indirectly, and 2V is the time to \\
    & initialize the \tcode{distance} and \tcode{predecessor} ranges. \\
\hline
    \textit{Preconditions} &  \tcode{seed >= 0 && seed < size(vertices(g))} \\
     &  \tcode{size(distance) >= size(vertices(g))}; caller must pre-extend \\
...
\hline
    \textit{Return} & \tcode{distance[uid]} will be the shortest, weighted distance of vertex\_id \tcode{uid} from \tcode{seed}. \\
    & If \tcode{uid} is not connected to \tcode{seed} by any edges then it will have a value of \\
    & \tcode{numeric_limits<range_value_t<Distance>>::max()}. \\
    & \\
    & \tcode{predecessor[uid]} will have the predecing vertex\_id of \tcode{uid} in the weighted shortest path to \tcode{seed}. \\
    & If \tcode{uid} is not connected to \tcode{seed} by any edges then it will have a value of \\
    & \tcode{numeric_limits<range_value_t<Predecessor>>::max()}. \\
\hline
\end{tabular}}
...
\phil{There's a hidden complexity of O(E+4V), not just O(E+2V), because we're requiring the caller to pre-extend \tcode{distance} and \tcode{predecessor}
which also initializes the ranges. 
If there were customization points for \tcode{resize(c,n[,init])}, \tcode{clear(c)} and \tcode{reserve(c,n)} we could reduce the complexity to 
O(E+2V) overall.
} 

\phil{Describe the pros and cons of different kinds of queues} \\

The default weight function \tcode{weight_fn} returns a value of 1. When that is used, \tcode{distances[uid]} will have the shortest number of edges 
...
the \tcode{predecessor} parameter and has a complexity of O(E+V).
\begin{lstlisting}
template <adjacency_list              G,
          ranges::random_access_range Distance,
          class EVF   = std::function<ranges::range_value_t<Distance>(edge_reference_t<G>)>,
...
A fuller description of \tcode{csr_graph}, including its constructors, can be found in the \tcode{<csr_graph} header section.
